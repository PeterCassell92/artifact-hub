...
- **JSON logs** to stdout (captured by CloudWatch Logs via the ECS log driver).
- Every log line carries a **correlation id**: a request id generated at the ALB/edge or the
  Express middleware, propagated through `core`, and echoed to the MCP adapter (per JSON-RPC
  call). This ties an API/MCP request to its DB/S3/SES effects.
...
- Infra metrics via CloudWatch: ECS CPU/memory, ALB request count / 5xx / target latency, RDS
  connections/CPU/storage, S3 request metrics.
- Surfaced on **CloudWatch dashboards** (provisioned in the `observability` Terraform module).

## 3. Health & readiness (v1)

- `GET /healthz` — liveness (process up).
- `GET /readyz` — readiness: DB reachable + migrations applied. Wired to the **ALB target group**
  so unhealthy tasks are drained and rolling deploys only shift traffic to ready tasks (`08`).

## 4. Error tracking (v1)

- **Sentry** (or equivalent) in the backend and SPA for exception capture with the `requestId`
  attached, so a user-facing error links to server context. Sentry DSN via Secrets Manager.
...
- **OpenTelemetry** traces for API + MCP requests → an OTLP backend (or CloudWatch/X-Ray). Nice
  for latency breakdowns across DB/S3/SES; optional for v1, low effort to add given the
...
- ECS task unhealthy / restart loop; service can't reach desired count.
- RDS high CPU / low free storage / connection saturation.
- Outbox backlog growing (invitations/emails not draining) — signals SES/Auth0 trouble.
...
- **Backups**: RDS automated backups tuned; S3 versioning for accidental-overwrite protection.
- **CDN hardening**: CloudFront signed URLs/cookies if we move file delivery behind the CDN.
